#include "head.h"
	...
	ofstream globalinfo,localinfo,duration;
	/*------------------------------------------------*/
	...
	{
cout<<random_binomial(10000,0.01)<<"     "<<random_binomial(10000,0.1)<<"     "<<random_binomial(10000,0.2)<<"     "<<random_binomial(10000,0.3)<<"     "<<random_binomial(10000,0.4)<<"     "<<random_binomial(10000,0.5)<<"     "<<random_binomial(10000,0.6)<<"     "<<random_binomial(10000,0.7)<<"     "<<random_binomial(10000,0.8)<<"     "<<random_binomial(10000,0.9)<<"     "<<random_binomial(10000,0.99)<<endl;
...
/*		int relpatches[Site.size()];
		int reltimes[Site.size()];
		for(int ii=0;ii<Site.size();ii++)relpatches[ii]=500000;*/
		ti.yearnow=0;
//cout<<"rel sites  "<<num_release_clusters<<endl;

	while (TT<ti.maxT+1)
			{
			pa.muAF=mortlist[TT];
			//pa.muAF=pa.muA;
//		cout<<"A   "<<num_release_clusters<<"   "<<Site.size()<<"    "<<to.CentSqVils<<endl;
			if(TT%365==0)
				{
				...
				if(num_release_clusters==1){
					...
					for(int jj=0;jj<num_release_clusters;jj++)
						...
						//cout<<"relpat  "<<relpat<<"   "<<uniquepat<<endl;
							};
...
//		cout<<"B"<<endl;
	...
	void ResetSites(void){
		...
		for(int cc=0;cc<Site.size();cc++)
			...
			Site[cc].distOrigin=dist(Site[site].x,Site[site].y,Site[cc].x,Site[cc].y);
			if(pa.alpha0>0)Site[cc].alpha0=exp(random_normal(pa.mu,pa.sig));else Site[cc].alpha0=0;
		//	cout<<Site[cc].alpha0<<"   "<<pa.mu<<"   "<<pa.sig<<"    "<<pa.al0var<<endl;
			};		
		if(pa.singlepop==0)
				{ for(int i=0;i<Site.size();i++)	SitesPopulate(i); };
		if(pa.singlepop==1)SitesPopulate(0); 
		if(pa.singlepop==2)
				{
				int relpat;
				int uniquepat=0;
				while(uniquepat==0)
					...
					uniquepat=1;
					relpat=rg.IRandom(0,Site.size()-1);
					if(Site[relpat].CentSq==0)uniquepat=0;
					};
cout<<relpat<<endl;
				SitesPopulate(relpat);
				};
...
//cout<<"pdisp  "<<pdisperse<<"     "<<Site[pat].area<<endl;
		...
		for(pat=0;pat<Site.size();pat++)
				{
		//if(pa.EDGEkernel==1) { pdisperse=pa.EDGEd*Site[pat].EDGETotW/Site[pat].area; };
		//if(pa.EDGEkernel==2) { pdisperse=pa.EDGEd*exp(-1*Site[pat].area/pa.LD); };
				pdisperse=pa.EDGEd*Site[pat].EDGETotW/Site[pat].area;
//cout<<pat<<"   "<<pdisperse<<"   "<<pdisperse*Site[pat].F[0][0]<<endl;
				for(int i=0;i<NumGen;i++)
				{
					Site[pat].MoveM[i]=random_binomial(Site[pat].M[i],pdisperse);
					Site[pat].M[i]-=Site[pat].MoveM[i];
...
void UpdateComp(int day){
	...
	if(!(Site[pat].comp+1>0)){cout<<"uc  "<<Site[pat].comp<<"     "<<pat<<"   rain  "<<rain[Site[pat].sqx][Site[pat].sqy][week]<<"  week   "<<week<<" alpha1  "<<pa.alpha1<<" Jtot  "<<Site[pat].JTot<<" al  "<<al<<endl;exit(0);};
